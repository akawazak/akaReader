...
6. The backend translates those requests into Suwayomi GraphQL/REST calls and caches some responses in memory. Manga-detail cache misses actively run Suwayomi's `fetchChapters`; explicit chapter refreshes and library update scans use `?force=1` to bypass the ten-minute manga cache and fail visibly if the source could not be checked. Chapter upload timestamps are normalized to ISO strings for consistent renderer formatting.
7. Reader progress, categories, history, settings, and library state are persisted in `localStorage`. Page progress and reading-time checkpoints are written synchronously before React state propagation so window hiding, reader teardown, and Alt+F4 cannot strand a debounced final page. Before Electron hides or quits the window, it asks the renderer to flush the active reader and waits for an acknowledgement with a bounded timeout. Continue targets are resolved by normalized chapter ID and resume the exact saved page; a removed provider chapter safely falls back to the first reading chapter.
8. Offline chapter pages are persisted in IndexedDB with compact source/title/chapter metadata and rehydrated as blob URLs when reopened. Downloads exposes exact stored chapter/page/byte totals, per-title cleanup, delete-read, delete-all, and origin quota usage; the existing auto-delete-read setting uses the same cleanup path. The serializable download queue is separately persisted in `localStorage`; an interrupted active job becomes pending after relaunch, resumes only after the backend is healthy, retries transient failures up to three times, and checks available browser storage before fetching a chapter. Manga details can export one chapter through a native CBZ save dialog or export every chapter to a chosen folder without buffering the archive in the renderer.
...
13. Settings exposes a main-process system-health check for the local API, Java, Suwayomi, managed JAR, writable/free data storage, and the optional protected-source helper. Automatic repair only reinstalls or restarts components that diagnostics show are needed. Backup export/import uses native dialogs, a size-limited versioned schema, an allowlist of restorable state, and compatibility with the older v2 export. It includes the chapter-update baseline and last-check summary: the first successful scan records known chapter IDs without calling an old unread backlog "new," later scans retain genuinely discovered unread chapters with their release metadata, and per-title failures remain visible without erasing earlier notifications. Users can choose manual, 15-minute, 30-minute, hourly, three-hour, or six-hour checks, enable desktop notifications, and opt into automatic offline downloads. App updates create and rotate private safety backups before download and install; offline image blobs are intentionally not included.
14. Library categories can be combined with smart views for Continue, Unread, Updated, Offline, and Completed titles. Smart views derive from stable progress, chapter IDs, update state, offline keys, and category state rather than parsing display chapter labels.
15. Release packages copy backend entry files and backend `node_modules` through separate `extraResources` mappings. CI runs `check:packaged-runtime` against unpacked Windows and Linux apps so a release cannot silently ship without the packages needed to bind port 3001, then launches each unpacked app and requires port 3001 to open. Before the Linux Xvfb launch, CI restores Electron's required root ownership and `4755` mode on the unpacked `chrome-sandbox`; it never disables Chromium's sandbox to make the test pass. Electron repeats the same backend-runtime preflight before forking, retries unexpected crashes with bounded exponential backoff, and stops a crash loop with a repair-oriented service issue instead of restarting forever.
16. Discord Rich Presence is off by default. When enabled from Settings, Electron connects locally to a running Discord desktop client and publishes only the generic `Browsing manga` or `Reading manga` state. It never publishes titles, chapters, sources, history, tokens, or URLs; disconnecting Discord remains non-fatal and is retried in the background while the option stays enabled.
