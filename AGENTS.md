...
13. Release packages copy backend entry files and backend `node_modules` through separate `extraResources` mappings. CI runs `check:packaged-runtime` against unpacked Windows and Linux apps so a release cannot silently ship without the packages needed to bind port 3001, then launches each unpacked app and requires port 3001 to open. The Linux smoke test runs under Xvfb. Electron repeats the same backend-runtime preflight before forking, retries unexpected crashes with bounded exponential backoff, and stops a crash loop with a repair-oriented service issue instead of restarting forever.
